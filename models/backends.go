...
func init() {
	...
	if ETCDDOMAIN == "" {
		ETCDDOMAIN = "etcd.mytest.sdp"
	}
	//	fmt.Println(ETCDDOMAIN)
	ETCDACCOUNT = beego.AppConfig.String("etcd_account")
...
var DnsadminTransport etcdClient.CancelableTransport = &http.Transport{
...
func (backend *Backend) DeleteKey(key string) (bool, error) {
	if backend.client == nil {
		if err := backend.SetMachines(nil); err != nil {
			return false, err
		}
	}
	_, err := backend.client.Delete(context.Background(), key, nil)
	if err != nil {
		return false, err
	}
	return true, nil
